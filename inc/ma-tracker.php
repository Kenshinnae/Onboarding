...
add_action( 'admin_post_co_ma_save', function () {
	...
	if ( 'client' === $entity ) {
		...
		$client_id = absint( $_POST['client_id'] ?? 0 ); $name = sanitize_text_field( wp_unslash( $_POST['project_name'] ?? '' ) );
...
function co_ma_client_detail_page() {
	...
	<div class="wrap co-ma-wrap co-ma-detail">
		...
		<section class="co-ma-section"><header><div><h2><?php echo esc_html( $client->post_title ); ?> projects</h2><p>No projects from other companies appear here</p></div><button class="co-ma-btn" data-ma-open="project">+ Add project</button></header><div class="co-ma-projects"><?php foreach ( $projects as $project ) : ?><article><span class="co-ma-project-icon">P</span><div><h3><?php echo esc_html( $project->post_title ); ?></h3><p><?php echo esc_html( co_ma_meta( $project->ID, 'description' ) ); ?></p></div><strong><?php echo esc_html( co_ma_hours( co_ma_used_hours( $id, $project->ID, 'monthly_retainer' === $package_type ? wp_date( 'Y-m' ) : '' ) ) ); ?><small>hours</small></strong></article><?php endforeach; ?></div></section></div>
	...
	<div class="wrap co-ma-wrap">
		...
		<div class="co-ma-layout">
			...
			<main class="co-ma-main"><?php if ( $selected ) : ?>
				...
				<section class="co-ma-section"><header><div><h2>Projects</h2><p><?php echo 'monthly_retainer' === co_ma_package_type( $selected_id ) ? 'Hours used this month by project' : 'Hours used by project'; ?></p></div><button class="co-ma-link" data-ma-open="project">+ Add project</button></header><div class="co-ma-projects"><?php if ( $projects ) : foreach ( $projects as $project ) : $project_hours = co_ma_used_hours( $selected_id, $project->ID, 'monthly_retainer' === co_ma_package_type( $selected_id ) ? wp_date( 'Y-m' ) : '' ); ?><article><span class="co-ma-project-icon">P</span><div><h3><?php echo esc_html( $project->post_title ); ?></h3><p><?php echo esc_html( co_ma_meta( $project->ID, 'description', 'No description' ) ); ?></p></div><strong><?php echo esc_html( co_ma_hours( $project_hours ) ); ?><small>hours</small></strong></article><?php endforeach; else : ?><p class="co-ma-empty">No projects yet. Add Project A, Project B or any client workstream.</p><?php endif; ?></div></section>
...
function co_ma_modals( $clients, $projects, $selected_id, $frontend = false ) { $action = esc_url( admin_url( 'admin-post.php' ) ); $return_url = $frontend ? add_query_arg( 'client', $selected_id, home_url( '/ma-tracker/' ) ) : ( ( $_GET['page'] ?? '' ) === 'ma-client-detail' ? admin_url( 'admin.php?page=ma-client-detail&client=' . $selected_id ) : '' ); ?>
