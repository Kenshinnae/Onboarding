...
function co_ma_modals( $clients, $projects, $selected_id, $frontend = false ) { $action = esc_url( admin_url( 'admin-post.php' ) ); $return_url = $frontend ? add_query_arg( 'client', $selected_id, home_url( '/ma-tracker/' ) ) : ( ( $_GET['page'] ?? '' ) === 'ma-client-detail' ? admin_url( 'admin.php?page=ma-client-detail&client=' . $selected_id ) : '' ); $pipeline_projects = co_ma_available_pipeline_projects(); ?>
	...
	<div class="co-ma-modal" data-ma-modal="project"><div><button class="co-ma-close">×</button><span class="co-ma-kicker">New workstream</span><h2>Add project</h2><form method="post" action="<?php echo $action; ?>"><?php wp_nonce_field( 'co_ma_save' ); ?><input type="hidden" name="action" value="co_ma_save"><input type="hidden" name="entity" value="project"><input type="hidden" name="return_url" value="<?php echo esc_url( $return_url ); ?>"><label>Client *<select required name="client_id"><?php foreach ( $clients as $client ) : ?><option value="<?php echo $client->ID; ?>" <?php selected( $selected_id, $client->ID ); ?>><?php echo esc_html( $client->post_title ); ?></option><?php endforeach; ?></select></label><label class="co-ma-pipeline-source">Project source<select name="pipeline_project_id" id="co-ma-pipeline-project"><option value="">Create a manual workstream</option><?php foreach ( $pipeline_projects as $pipeline_project ) : $company = co_crm_meta( $pipeline_project->ID, 'company' ); ?><option value="<?php echo $pipeline_project->ID; ?>" data-title="<?php echo esc_attr( $pipeline_project->post_title ); ?>" data-description="<?php echo esc_attr( co_crm_project_brief( $pipeline_project->ID ) ?: co_crm_meta( $pipeline_project->ID, 'current_task' ) ); ?>"><?php echo esc_html( ( $company ?: 'Client' ) . ' — ' . $pipeline_project->post_title ); ?></option><?php endforeach; ?></select><small>Shows every unlinked Pipeline project in MA status. The source company is included in each option.</small></label><label>Project name *<input required name="project_name" placeholder="e.g. Website maintenance"></label><label>Description<textarea name="description" rows="4"></textarea></label><div class="co-ma-form-actions"><button class="co-ma-btn co-ma-btn-light co-ma-cancel" type="button">Cancel</button><button class="co-ma-btn" type="submit">Add project</button></div></form></div></div>
...
function co_ma_assets() { ?>
	...
	<script>document.addEventListener('DOMContentLoaded',()=>{const modal=document.querySelector('[data-ma-modal="project"]'),source=modal?.querySelector('#co-ma-pipeline-project'),name=modal?.querySelector('[name="project_name"]'),description=modal?.querySelector('[name="description"]');if(!source)return;source.addEventListener('change',()=>{const option=source.selectedOptions[0];if(!option?.value)return;if(name)name.value=option.dataset.title||'';if(description)description.value=option.dataset.description||''})});</script>
